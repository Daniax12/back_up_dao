...
public class BddObject {
     ...
     * @throws Exception 
     */
    public static <T> T findById(String nameTable, Object object, Connection connection) throws Exception{
        boolean isOpen = false;
        ConnectionBase connectionBase = new ConnectionBase();
        if(connection == null){
            connection = connectionBase.dbConnect();     // If it is null, creating connection
        ...
        try { 
            ...
            if(pkValue == null) throw new Exception("Primary key undefined. Error on "+myPk.getName());
            
            List<T> allObject = find(nameTable, object, connection);               // Pk already set, the function 'find' has selection function
            System.out.println("===============================================");
            if(allObject.size() == 0) return null;
            return allObject.get(0);
        } catch (Exception e) {
            e.printStackTrace();
            throw new Exception("Error on getting the object "+object.getClass().getSimpleName()+ " by its id. Error: "+e.getMessage());
        } finally{
            if(isOpen == false) connection.close();
        }        
    }
     ...
     * 
     */
    public static <T> List<T> findByOrder(String nameTable, Object object, String nameAttribute, Ordering ordering, Connection connection) throws Exception{
        ...
        try { 
            ...
            String sql = SqlScriptUtil.sqlScriptSelectingTableOrder(nameTable, myFields, notNullField, object, nameAttribute, ordering);                     // Get the sql
     ...
     * 
     */
    public static <T> List<T> find(String nameTable, Object object, Connection connection) throws Exception{
        List<T> result = new ArrayList<>();
        boolean isOpen = false;
        ConnectionBase connectionBase = new ConnectionBase();
        if(connection == null){
        ...
        try { 
            ...
            List<Field> myFields = FieldUtil.fieldsToList(object);                                               // Get all fields
            List<Field> notNullField = FieldUtil.FieldValueNotNull(object, myFields);           // All fields that are not null
            String sql = SqlScriptUtil.sqlScriptSelectingTable(nameTable, notNullField, object);                     // Get the sql
            stm = BddObject.psUtil(stm, connection, notNullField, sql, object);
